...
```{r FAP_SBS_mutational_signature_burden}
...
```
```{r FAP_SBS_mutational_signature}
ggplot(SBS_sig_proportion[SBS_sig_proportion$group=='FAP',], aes(x = sample, y = E2, fill = E1)) + geom_bar(stat = "identity") +
  ...
  theme(text = element_text(size = 6),
    ...
    scales = "free_x", space = "free")
```
```{r FAP_ID_mutational_signature_burden}
...
```{r Control_SBS_mutational_signature_burden}
# SBS
sig_order=1:10
names(sig_order)=c("SBS1","SBS5","SBS18","SBS2","SBS13","SBS88",'SBS89','SBSC',"SBS17b",'SBS93')
...
```{r Control_SBS_mutational_signature }
...
```{r Control_ID_mutational_signature_burden}
...
```{r Control_ID_mutational_signature}
