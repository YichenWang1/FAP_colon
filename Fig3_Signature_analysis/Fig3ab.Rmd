...
```{r FAP SBS Mutational signature + burden}
...
```
```{r FAP SBS Mutational signature }
ggplot(SBS_sig_proportion[SBS_sig_proportion$group=='FAP',], aes(x = sample, y = E2, fill = E1)) + geom_bar(stat = "identity") +
  ...
  theme(text = element_text(size = 6),
    ...
    scales = "free_x", space = "free")
```
```{r FAP ID Mutational signature + burden}
...
```{r Control SBS Mutational signature + burden}
# SBS
sig_order=1:10
names(sig_order)=c("SBS1","SBS5","SBS18","SBS2","SBS13","SBS88",'SBS89','SBSC',"SBS17b",'SBS93')
...
```{r Control SBS Mutational signature }
...
```{r Control ID Mutational signature + burden}
...
```{r Control ID Mutational signature}
